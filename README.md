# JWT Auth App (Flask + PostgreSQL + Redis + Pydantic v2 + Celery + Gmail SMTP)

A production-ready, highly secure RESTful Web API and Web Application built with **Flask**, **PostgreSQL** (Flask-SQLAlchemy), **Redis**, **Pydantic v2**, **Celery**, **PyJWT**, and **Gmail SMTP Email Verification**.

Features user authentication, secure **HTTP-Only cookies**, **Gmail Email Verification Links**, **Password Reset Web Flow**, instant **Redis token revocation**, **Role-Based Access Control (RBAC)**, **Pydantic payload validation**, **Redis query caching**, **asynchronous task queues**, team contact management (Fellows), file attachments, analytics, and security audit logging.

---

## 🚀 Key Features

- 📩 **Gmail Email Verification Link Flow:** New users verify their account by clicking a button in their Gmail inbox (`/verify-email?token=...`).
- 🔑 **Gmail Password Reset Web Flow:** Users reset passwords directly from their Gmail inbox via a web landing page (`/reset-password?token=...`).
...
- ⚡ **Asynchronous Task Queue (Celery):** Background email processing offloaded to Celery workers using Redis as broker and `smtplib` for TLS Gmail delivery.
...
# Gmail SMTP Configuration (Use 16-character Google App Password)
...
## 5. Key Web Routes & API Specification

### Web Pages
- `GET /` or `/signin` — Sign In & Forgot Password Request
...
### REST API Endpoints
All protected API endpoints require `Authorization: Bearer <JWT_TOKEN>` header (or HTTP-Only refresh cookie).

| Method | Endpoint | Auth Required | Role Required | Description |
|---|---|:---:|:---:|---|
| POST | `/api/v1/auth/signup` | No | — | Register new user account & dispatch Gmail verification email |
| POST | `/api/v1/auth/signin` | No | — | Authenticate user (Requires verified email) |
| POST | `/api/v1/auth/verify-email` | No | — | Confirm email address with verification token |
...
| POST | `/api/v1/auth/forgot-password` | No | — | Request password reset email (dispatches async Celery task) |
| POST | `/api/v1/auth/reset-password` | No | — | Confirm password reset with reset token & new password |
...
```
jwt_auth_app/
├── app.py                 # Flask App Factory (registers blueprints, db initialization & web routes)
├── run.py                 # Server entry point (loads .env, starts Flask app)
├── config.py              # Application configuration (DATABASE_URL, REDIS_URL, MAIL_*, JWT settings)
...
├── tasks/                 # Celery task queue (celery_app.py, email_tasks.py with Gmail SMTP)
...
├── templates/             # HTML templates (signin, signup, profile, verify_email, reset_password)
├── static/                # CSS styles
